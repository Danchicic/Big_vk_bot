...
def reader(vk_session, user, event, i, cursor, conn):
    print(event.user_id, 'reader')
    cursor.execute("UPDATE user_state SET reader=? WHERE id=?", (True, event.user_id))
    conn.commit()
    ...
    if txt == 'читатель':
        ...
        # if ['Вы выбрали читателя', create_reader_kb] not in user_state['data']['stack']:
        #     user_state['data']['stack'].append(['Вы выбрали читателя', create_reader_kb])
        cursor.execute("INSERT INTO user_state (moder) VALUES (True)")
    elif txt == 'качество перевода':
        i += 1
        user.send_msg(vk_session=vk_session, user_id=event.user_id,
                      text='Перевожу вас на общение с модератором',
                      keyboard=pass_kb)
        cursor.execute("UPDATE user_state SET moder=? WHERE id = ?", (True, event.user_id))
    ...
    #     i += 1
    #     cursor.execute("INSERT INTO user_state (reader) VALUES (False)")
    #     cursor.execute("INSERT INTO user_state (moder) VALUES (False)")
    #     user.send_msg(vk_session=vk_session, user_id=event.user_id,
    ...
    #     user_state['data']['stack'].append(['Выберите из предложенного списка', create_reader_kb])
        ...
        user.send_msg(vk_session, event.user_id, text='Опишите фичу',
                      keyboard=pass_kb)
        cursor.execute("INSERT INTO user_state (click_up) VALUES (True)")
    ...
    # elif user_state['data']['click_up']:  # fgjklasdghjklfdshjgklfdshjgkljdaklsfjdlkajflkadsjfkla
    ...
    # 5) Не пришли деньги
        ...
        cursor.execute("INSERT INTO user_state (money) VALUES (True)")
    ...
    # не пришли деньги
    # elif user_state['data']['money'] and txt == 'да':  # fjsdklgjfiksdjgklfdsjgklfdsjlgkfsjklgsjkl;
    #     i += 1
    #     user.send_msg(vk_session=vk_session,
    #                   user_id=event.user_id,
    #                   text='Привязан к re?',
    ...
    #     cursor.execute("INSERT INTO user_state (link_re, money) VALUES (True, False)")

    # user_state['data']['link_re'] = True
    # user_state['data']['money'] = False

    # elif user_state['data']['money'] and txt == 'нет':  # fgjsklgjfklsjglkfsdjgklsfdjl;k
    #     i += 1
    #     user.send_msg(vk_session=vk_session,
    #                   user_id=event.user_id,
    ...
    #     if ['Прошло 24 часа?', if_kb_reader_24] not in user_state['data']['stack']:
    #         user_state['data']['stack'].append(['Прошло 24 часа?', if_kb_reader_24])
    #     cursor.execute("INSERT INTO user_state (money) VALUES (False)")
    #     user_state['data']['money'] = False

    # Привязан к re?
    # elif txt == 'да' and user_state['data']['link_re']:  # fdhjsfkhjksgfhjksghfjksl
    #     i += 1
    #     user.send_msg(vk_session=vk_session,
    #                   user_id=event.user_id,
    ...
    #     cursor.execute("INSERT INTO user_state (link_re) VALUES (False)")
    #
    ...
    #     cursor.execute("INSERT INTO user_state (moder, link_re, linked) VALUES (False, False, True)")
    #     moder = False
    #     user_state['data']['link_re'] = False
    ...
    #                   text='Идет пересчет баланса...',
    #                   keyboard=pass_kb)
    #     cursor.execute("INSERT INTO user_state (linked) VALUES (False)")
    ...
    #     i += 1
    #     moder = True
    #     cursor.execute("INSERT INTO user_state (moder, linked) VALUES (True, False)")
    #     user.send_msg(
    #         vk_session=vk_session,
    #         user_id=event.user_id,
    #         text='Перевожу вас на общение с модератором',
    ...
    #         user_state['data']['stack'].append(['Привязать к re?', if_kb_reader_linked])
        ...
        user.send_msg(vk_session=vk_session,
                      ...
                      keyboard=if_kb_reader_re)
        # user_state['data']['link_re'] = True
        cursor.execute("INSERT INTO user_state (link_re) VALUES (True)")
        # if ['не пришли деньги/тикеты', money_kb] not in user_state['data']['stack']:
        #     user_state['data']['stack'].append(['не пришли деньги/тикеты', money_kb])
    if txt in ['у вас остались вопросы?',
        ...
        cursor.execute("UPDATE user_state SET reader=? WHERE id=?", (False, event.user_id))
        conn.commit()
